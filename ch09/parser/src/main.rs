...
impl Ast {
    ...
    fn uniop(op: UniOp, e: Ast, loc: Loc) -> Self {
        Self::new(AstKind::UinOp { op, e: Box::new(e) }, loc)
    }
...
fn parse_left_binop<Tokens>(
    ...
    subepxr_parser: fn(&mut Peekable<Tokens>) -> Result<Ast, ParseError>,
    op_parser: fn(&mut Peekable<Tokens>) -> Result<BinOp, ParseError>,
) -> Result<Ast, ParseError>
where
    Tokens: Iterator<Item = Token>,
{
    let mut e = subepxr_parser(tokens)?;
...
impl StdError for InterpreterError {
    fn description(&self) -> &str {
        ...
        match self.value {
            DivisionByZero => "the right hand expression of division evaluates to zero",
        }
    }
}
